...
_LIGHTER = {Attention.ENGAGED: Attention.ALERT,
...
class AdaptiveScheduler:
    ...
    def _update_attention(self, risk: float, min_conf: float) -> str:
        ...
        elif (0.0 < min_conf < cfg.conf_second_opinion
        ...
        if risk < cfg.risk_alert:
            self._calm_frames += 1
            if (self._calm_frames >= cfg.cooldown_frames
                    and self.attention != Attention.PATROL):
                self.attention = Attention.PATROL
                self._calm_frames = 0
    ...
    def _update_resource(self, res: ResourceSnapshot) -> None:
        cfg = self.cfg
        starving = ((0 < res.fps < cfg.min_fps)
                    or res.gpu_percent >= cfg.gpu_high)
        ...
        if starving:
            ...
            if self._healthy_frames >= cfg.resource_recovery_frames:
                self.resource = (Resource.CONSTRAINED if constrained
    ...
    # ------------------------------------------------------------------
    def decide(self, risk: float, min_detection_conf: float,
               resources: ResourceSnapshot) -> Decision:
        prev_mode = self.mode
        ...
        if self.resource == Resource.CRITICAL:
            ...
            reason = (f"compute CRITICAL (fps={resources.fps:.1f}); "
                      f"attention demand stays {self.attention.value}")
        else:
            eff = self.attention
            if (self.resource == Resource.CONSTRAINED
                    and eff != Attention.ENGAGED):
        ...
        self.mode = mode
        decision = Decision(mode=mode, models=models, reason=reason,
                            switched=(mode != prev_mode),
                            timestamp=time.time(),
                            attention=self.attention, resource=self.resource)
    ...
    def switch_log(self) -> List[dict]:
        return [{"t": d.timestamp, "mode": d.mode.value,
                 "attention": d.attention.value, "resource": d.resource.value,
                 "reason": d.reason} for d in self.history]
