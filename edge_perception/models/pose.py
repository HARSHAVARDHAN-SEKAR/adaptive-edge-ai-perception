...
def pose_center(kp_xy: np.ndarray, kp_conf: np.ndarray, thr: float = 0.3) -> np.ndarray:
...
class YoloPose(PerceptionModel):
            ...
            for kp, b in zip(res.keypoints, res.boxes):
                ...
                out.poses.append(
                    PoseKeypoints(
                        ...
                        keypoints_xy=xy,
                        keypoints_conf=cf,
                        activity=classify_activity(xy, cf),
                    )
                )
        return out
    ...
    def _infer_mock(self, frame_bgr: np.ndarray) -> ModelOutput:
        ...
        for obj in mock_world(frame_bgr):  # skeleton INSIDE the person box
            ...
            xy = np.stack(
                [
                    np.full(17, cx) + rng.normal(0, (x2 - x1) * 0.08, 17),
                    np.linspace(y1 + 0.05 * (y2 - y1), y2 - 0.02 * (y2 - y1), 17),
                ],
                axis=1,
            )
