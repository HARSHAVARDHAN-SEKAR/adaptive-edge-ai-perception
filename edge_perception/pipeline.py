...
from .models import (  # noqa: F401 (registers models)
    depth,
    detection,
    pose,
    segmentation,
...
from .scheduler.model_selector import AdaptiveScheduler, Decision, Mode, SchedulerConfig
...
class FrameResult:
    ...
    narration: str | None = None  # last VLM scene narration
    ...
    def to_dict(self):
        return {
            ...
            "model_latencies_ms": {
                k: round(v, 2) for k, v in self.model_latencies.items()
            },
            "decision_confidence": (
                round(self.decision_confidence, 3)
...
class PerceptionPipeline:
    def __init__(
        ...
        backend: str = "auto",
        adaptive: bool = True,
        fixed_models: list[str] | None = None,
        ...
        self.fixed_models = fixed_models or [
            "yolo_large",
            "yolo_nano_seg",
            "midas_small",
            "yolo_pose",
        ...
        self._narration_frame: int | None = None
    ...
    def process(self, frame: np.ndarray, frame_index: int = 0) -> FrameResult:
        ...
        if self.adaptive:
            decision = self.scheduler.decide(
                self._last_risk, self._last_min_conf, resources
            )
        else:
            decision = Decision(
                Mode.ENGAGED,
                list(self.fixed_models),
                "fixed baseline",
                False,
                time.time(),
            )
        ...
        if (
            ...
            if self._skip_counter % self.degraded_frame_skip != 0:
                ...
                result = FrameResult(
                    ...
                    model_latencies={},
                    decision_confidence=prev.decision_confidence,
                    narration=self.narration,
                    skipped=True,
                )
                if self._log:
        ...
        if (
            self.vlm_on_engagement
            and decision.switched
        ...
        if (
            ...
            and frame_index - self._narration_frame > self.narration_ttl_frames
        ):
            self.narration = None
            self._narration_frame = None
