...
def run_system(
    label, n_frames, adaptive, fixed=None, backend="mock", seed=7, events=None
):
    cost = {n: model_base._REGISTRY[n].cost for n in model_base.available()}
    events = events or mission_events(n_frames)
    pipe = PerceptionPipeline(adaptive=adaptive, fixed_models=fixed, backend=backend)
    ...
    frames = SyntheticSource(
        size=(120, 160), n_frames=n_frames, event_windows=events, seed=seed
    ).frames()  # streaming generator
    ...
    pipe.close()
    lat = np.array(lat)
    fps_series = np.array(fps_series)
    return {
        "system": label,
        ...
        "event_full_suite_coverage": (
            round(full_suite_frames / event_frames, 2) if event_frames else 0.0
        ),
        "high_risk_full_suite_coverage": (
            round(high_risk_covered / high_risk_frames, 2) if high_risk_frames else 0.0
        ),
...
def main():
    ...
    rows = [
        ...
        run_system(
            "adaptive", args.frames, adaptive=True, backend=args.backend, events=events
        ),
    ]

    hdr = (
        "| System | Mean FPS | FPS σ | Lat mean (ms) | Lat p95 (ms) "
    ...
    if heavy["cost_proxy_units"]:
        ...
        lines += [
            "",
            (
                f"Adaptive uses **{saving:.0f}% fewer cost-proxy units** "
