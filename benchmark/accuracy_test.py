...
def main():
    ...
    ap.add_argument("--images", default="assets",
                    help="folder of jpg/png test images")
    ...
    ref = model_base.create(args.reference, device=args.device,
                            backend=args.backend)
    gts = [ref(f).detections for f in frames]
    ref.unload()

    lines = [f"# Tier agreement with {args.reference} pseudo-labels "
